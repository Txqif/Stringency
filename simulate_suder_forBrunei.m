...
% identification_window is the window of days in the future used to estimate the model parameters on one of the update days
...
for i=start_day:end_day
    ...
    if i == update_days(d)
        ...
        for n = 1:Np
            if estimate_params(n)&&d~=1
                nlgr.Parameters(n).Minimum =0;
                nlgr.Parameters(n).Maximum = 10;
            else
